...
const AccomPackageForm = () => {
    ...
    const onSubmit=async formData=>{
        ...
        try {
            let {data}=await axios.post(`${API_URL}/api/v1/accomTypes`,request,{ headers: { Authorization: `Bearer ${userDetails.token}` }})
            if(!data){
    ...
    return (
        <>
        <Content title='Add new Accomodation Packages ' 
                 subtitle={title}
                 widget={()=>{}}>
...
export default AccomPackageForm
