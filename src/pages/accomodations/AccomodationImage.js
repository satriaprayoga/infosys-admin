...
const AccomodationImage = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            let { data } = await axios.post(`${API_URL}/api/v1/images/accomodation/${state.accomodation.id}`, formData);
            if (!data) {
    ...
    const fetchImages = async () => {
        try {
            setLoading(true);
            let { data } = await axios.get(`${API_URL}/api/v1/images/accomodation/all/${state.accomodation.id}`);
            if (!data) {
    ...
    return (
        <>
        {loading && <Loading />}
       <Content title={`Image Cover for ${state.accomodation.name}`}
           widget={() => { }}
           >
          
           <div className="row">
               <div className="col-6">
                   <img className="img-fluid img-thumbnail" src={`${API_URL}/api/v1/images/accomodation/${state.accomodation.id}`} alt="User profile picture" />
